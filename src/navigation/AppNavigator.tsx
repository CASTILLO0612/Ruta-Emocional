...
export const AppNavigator: React.FC = () => {
  const { isAuthenticated, isLoading, role, initializeAuth } = useAuthStore();

  useEffect(() => {
    const unsubscribe = initializeAuth();

    import('../scripts/seedData').then(({ seedAllPsychologists }) => {
      seedAllPsychologists().catch((err) => console.warn('[Seed Error]', err));
    });
